...
graph TD
    ...
    CURL[libcurl]
    RESULT[corelib/dbj_result.h]
    DEFER[corelib/dbj_defer.h]
    LOG[corelib/dbj_simple_log.h]
    INTRO[corelib/dbj_clintro.h]
...
`corelib/` knows nothing of this folder -- the dependency is one way
...
`DBJ_MAKERESULT` from `corelib/dbj_result.h` generates the tagged
...
`corelib/dbj_defer.h` (Gustedt's `defer`, built on `[[gnu::cleanup]]`)
