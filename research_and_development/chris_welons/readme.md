...
Wellons' `Str` lands in the repo as [../../corelib/dbj_str_slice.h](../../corelib/dbj_str_slice.h), renamed `dbj_str_slice`.

It does **not** replace `DEFINE_DBJSTR_TYPE` in [../../corelib/dbj_str.h](../../corelib/dbj_str.h). The two are different kinds of a thing:
...
[../../corelib/dbj_required_compile_time.h](../../corelib/dbj_required_compile_time.h) first in the include list — GCC 15+, Clang rejected. `nullptr`, `bool`, `constexpr`, `[[maybe_unused]]` where they fit. Wellons did not mandate a standard; here it is not optional.
...
[../../corelib/dbj_defer.h](../../corelib/dbj_defer.h) on the single `malloc` backing the arena in `main`. That is the only allocation in the program; everything else is bump-allocated out of it and freed by scope, not by a call.

### 5. `dbj_clintro`

One [../../corelib/dbj_clintro.h](../../corelib/dbj_clintro.h) call at the top of `main`.

### 6. `DBJ_LOOP_AS` — tried, then dropped

[../../corelib/dbj_macros.h](../../corelib/dbj_macros.h) was used at first, then taken out. It is an opinionated dbj macro, not a mandatory one, and here it bought nothing: its counter is `size_t`, so both `clone` loops had to cast their `ptrdiff_t` bound going in, and the demo loops had to cast their counter coming out. Plain `for` needs neither.
...
[../../corelib/dbj_result.h](../../corelib/dbj_result.h) carries two 512-byte char arrays per error. Right at a CRUD app's boundary; wrong inside a hot `lookup`. Deferred until after release 1.0. A comment marks the OOM `assert` as the place it would go.
...
[../../third_party/dbj_ubenchtest/](../../third_party/dbj_ubenchtest/) declares itself DEPRECATED in its own header; the active harness is `dbj_nanobench.h`.

→ **Reversed later, repo-wide.** ubenchtest is now the one benchmark harness and nanobench is the deprecated one — see [../../README.md](../../README.md#testing-and-benchmarking).
...
`DBJ_MAKERESULT(HashMapElement)` from [../../corelib/dbj_result.h](../../corelib/dbj_result.h), used unchanged — this reverses §11 of the 0.1 plan.
...
[../../deprecated/dbj_nanobench/dbj_nanobench.h](../../deprecated/dbj_nanobench/dbj_nanobench.h), following [../../deprecated/dbj_nanobench/dbj_nanobench_test.c](../../deprecated/dbj_nanobench/dbj_nanobench_test.c). This reverses §12 of the 0.1 plan — they are in the same translation unit after all, since nanobench only needs `DBJ_NANOBENCH_IMPLEMENTATION` defined once and this is a single-file program.
...
- **`dbj_str.h`'s `static sz` parameter.** `str32_create` demands a full 32 bytes, so no string literal can be passed; both this file and [../dbj_str_test/dbj_str_test.c](../dbj_str_test/dbj_str_test.c) work around it. Twice now. Not changed — corelib is left alone — but worth a decision of its own.
