...
# Linux kernel `cleanup.h` — reference only
...
repo, and it never will be — see `corelib/dbj_defer.h` for the `defer`
macro actually in use.

**Why it's here, not used directly**

- This repo already has its own `defer`: Jens Gustedt's
  `[[gnu::cleanup]]`-based macro in `corelib/dbj_defer.h`. C23 does not
...
- This repo is single-compiler by policy: GNU C (GCC) 15 or later,
  no portability shims for Clang/MSVC. The bulk of this file is kernel
  machinery (`DEFINE_FREE`, `CLASS`, `DEFINE_GUARD`,
  `DEFINE_LOCK_GUARD_*`, `ACQUIRE`/`ACQUIRE_ERR`, context-analysis
  attributes) built for kernel-only concerns — `spinlock_t`,
  `rcu_read_lock`, `errno`/`IS_ERR` conventions, `__no_context_analysis`
  — none of which this repo has or wants.
- Adopting it wholesale would mean importing a lot of kernel
  ...
  directly on top of `corelib/dbj_defer.h` when the mutex work in
...
static __always_inline __must_check const volatile void *__must_check_fn(const volatile void *val)
{
	return val;
}
