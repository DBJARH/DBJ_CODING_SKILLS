    ...
    do { \
        ...
        case CMD_UPDATE: \
            my_DBG("CMD UPDATE id=%u subject=%s", \
                   (cmd_)->params.update.id, (cmd_)->params.update.subject); \
            break; \
        case CMD_DELETE: \
...
#define EMAIL_ID_NULL  ((EmailId)0x00)  /* reserved — marks empty slot and null record */

/*
IMPORTANT!  since EMAIL_ID_NULL is  actually zero 
The whole logic will work, because EmailStorage email_storage = {} , when initialised in main()
will zero the storage array inside, so we do not need to initialize the empty storage to 'null emails' by hand
If in the future someone changes the EMAIL_ID_NULL to some other value, or other type,  this whole logic will not work
*/

static_assert(EMAIL_ID_NULL == 0,
    "EMAIL_ID_NULL must be 0 — EmailStorage relies on zero-init for empty-slot detection");
 ...
 * Remember: that was 1966, before OO anything
 * but still superior 2026, after OO everything
 * 
...
typedef struct
{
    ...
    union
    {
        struct { char to[64]; char from[64]; char subject[128]; char body[512]; } create;
        struct { EmailId id; }                                                    read;
        struct { EmailId id; char subject[128]; char body[512]; }                 update;
        struct { EmailId id; }                                                    del;
    } params;
} EmailCmd;
...
#define EMAIL_CREATE_CMD(to_, from_, sub_, body_) \
    &(EmailCmd){.tag = CMD_CREATE, .params.create = {(to_), (from_), (sub_), (body_)}}
#define EMAIL_READ(id_) \
    &(EmailCmd){.tag = CMD_READ,   .params.read   = {(id_)}}
#define EMAIL_UPDATE(id_, sub_, body_) \
    &(EmailCmd){.tag = CMD_UPDATE, .params.update = {(id_), (sub_), (body_)}}
#define EMAIL_DELETE(id_) \
    &(EmailCmd){.tag = CMD_DELETE, .params.del    = {(id_)}}
...
static EmailRecord *find_first_slot_by_email_id(EmailStorage *s, EmailId id)
{
    assert(id != EMAIL_ID_NULL && "find_first_slot_by_email_id called with null id");

    for (U8TYPE i = 0; i < CAPACITY; ++i)
        if (s->storage[i].id == id)
            return &s->storage[i];
    return nullptr;
}
...
{
    for (U8TYPE i = 0; i < CAPACITY; ++i)
        if (s->storage[i].id == EMAIL_ID_NULL)
            return &s->storage[i];
    return nullptr;
}
...
static void storage_dump(const EmailStorage *s)
{
    for (U8TYPE slot_idx = 0; slot_idx < CAPACITY; ++slot_idx)
    {
        ...
        if (r->id != EMAIL_ID_NULL)
            my_LOG(" slot[%03u] Emil id=[%-3u] to=%-22s subject=%s", slot_idx, r->id, r->to, r->subject);
        else
...
static Result impl_create(EmailStorage *s, const EmailCmd *cmd)
{
    ...
    if (!slot)
        return result_err(__func__, "storage full");
...
{
    EmailRecord *r = find_first_slot_by_email_id(s, cmd->params.update.id);
    if (!r)
        return result_err(__func__, "not found");
...
{
    EmailRecord *r = find_first_slot_by_email_id(s, cmd->params.del.id);
    if (!r)
        return result_err(__func__, "not found");
...
static EmailRecord email_dispatch(EmailStorage *s, const EmailCmd *cmd)
{
    ...
    switch (cmd->tag)
    {
    case CMD_CREATE: dispatch_result = impl_create(s, cmd); break;
    case CMD_READ:   dispatch_result = impl_read(s, cmd);   break;
    case CMD_UPDATE: dispatch_result = impl_update(s, cmd); break;
    case CMD_DELETE: dispatch_result = impl_delete(s, cmd); break;
    }
    ...
    EmailRecord *found = find_first_slot_by_email_id(s, dispatch_result.ok.id);
...
int main(void)
{
    ...
    {
    /* CREATE */
        EmailRecord e1 = email_dispatch(&db, EMAIL_CREATE_CMD("bob@example.com",   "alice@example.com", "Q1 report",     "Please find attached..."));
        assert(e1.id != EMAIL_ID_NULL && "CREATE bob failed");

        EmailRecord e2 = email_dispatch(&db, EMAIL_CREATE_CMD("carol@example.com", "alice@example.com", "Meeting notes", "As discussed...")        );
        assert(e2.id != EMAIL_ID_NULL && "CREATE carol failed");

        EmailRecord e3 = email_dispatch(&db, EMAIL_CREATE_CMD("dave@example.com",  "bob@example.com",   "Re: Q1 report", "Looks good.")            );
    ...
    /* READ */
        ...
        assert(r2.id == EMAIL_ID_NULL && "READ 0x42 must return null record — id never created");
        (void)r2;

    /* UPDATE */
        EmailRecord u1 = email_dispatch(&db, EMAIL_UPDATE(e1.id, "Q1 report (final)", "Revised."));
        if (u1.id == EMAIL_ID_NULL) my_LOG("UPDATE id=%u failed", e1.id);

        EmailRecord u2 = email_dispatch(&db, EMAIL_UPDATE(0x42, "Ghost", "Nobody home."));
        assert(u2.id == EMAIL_ID_NULL && "UPDATE 0x42 must return null record — id never created");
        (void)u2;
    ...
    /* DELETE */
        ...
        assert(d2.id == EMAIL_ID_NULL && "DELETE twice must return null record");
        (void)d2;

        EmailRecord gone = email_dispatch(&db, EMAIL_READ(e2.id));
        assert(gone.id == EMAIL_ID_NULL && "READ after DELETE must return null record");
        (void)gone;
    }
...
/*
 * original vibed version is much inferior to the above https://godbolt.org/z/sb7MTeGax
 * in the above human spent a lot of time manipulating claude to assist in that version
 *
 * A tribute to C.A.R. Hoare, "Record Handling" (1966).
 *
 * Hoare's paper predates Simula 67, Smalltalk, and the entire OOP and OOP
 ...
 *
 * Simula team read Hoare's paper, kept the subclass idea, and dropped
